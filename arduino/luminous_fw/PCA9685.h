...
#define PCA9685_ALL_ON_H            0xFB   //  used for allON()
...
public:
  ...
  uint8_t  setPWMAll(uint16_t onTime, uint16_t offTime);
  uint8_t  getPWM(uint8_t channel, uint16_t* onTime, uint16_t* offTime);
  uint8_t  getPWMAll(uint16_t* onTime, uint16_t* offTime);

  // single PWM setting, channel = 0..15, offTime = 0..4095  (onTime = 0)
  uint8_t  setPWM(uint8_t channel, uint16_t offTime);
  // all PWM setting, channel = 0..15, offTime = 0..4095  (onTime = 0)
  uint8_t  setPWMAll(uint16_t offTime);
