...
//  PWM  FREQUENCY
//
//  write value to all PWM channels
uint8_t PCA9685::setPWMAll(uint16_t onTime, uint16_t offTime)
{
  _error = PCA9685_OK;
  uint8_t reg = PCA9685_ALL_ON_L;
  //  mask both onTime and offTime to be in range 0..4095
  //  allows to set the FULL_ON / FULL OFF bits.
  return writeRegister2(reg, onTime & 0x1FFF, offTime & 0x1FFF);
}
...
//  write value to single PWM channel
uint8_t PCA9685::setPWMAll(uint16_t offTime)
{
  return setPWMAll(0, offTime);
}
...
//  read value from single PWM channel
//  may include FULL_ON or FULL_OFF bit.
uint8_t PCA9685::getPWMAll(uint16_t* onTime, uint16_t* offTime)
{
  _error = PCA9685_OK;
  return readRegister2(PCA9685_ALL_ON_L, onTime, offTime);
}
...
{
  _error = PCA9685_OK;
  writeRegister(PCA9685_ALL_ON_H, 0x00);
  return writeRegister(PCA9685_ALL_OFF_H, 0x10);
}

uint8_t PCA9685::allON()
{
  _error = PCA9685_OK;
  writeRegister(PCA9685_ALL_OFF_H, 0x00);
  return writeRegister(PCA9685_ALL_ON_H, 0x10);
}
